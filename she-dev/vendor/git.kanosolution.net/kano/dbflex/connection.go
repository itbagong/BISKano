...
type IConnection interface {
	...
	Execute(ICommand, codekit.M) (interface{}, error)
	Cursor(ICommand, codekit.M) ICursor
	...
	EnsureTable(string, []string, interface{}) error
	EnsureIndex(tableName, indexName string, isUnique bool, fields ...string) error

	BeginTx() error
	Commit() error
	RollBack() error
	DisableTx(disable bool)
	SupportTx() bool
	IsTx() bool
...
// BeginTx to start a transaction
func (b *ConnectionBase) BeginTx() error {
	b._isTx = true
	return nil
}

// Commit commit a transaction
func (b *ConnectionBase) Commit() error {
	b._isTx = false
	return nil
}

// RollBack cancel transaction and rolling back data to their state before it is being trx-ed
func (b *ConnectionBase) RollBack() error {
	b._isTx = false
	return nil
}

// IsTx check if a connection is in a tx mode or not
func (b *ConnectionBase) IsTx() bool {
	return false
}

// DisableTx to disable connection to use tx
func (b *ConnectionBase) DisableTx(disable bool) {
}

// SupportTx is this connection supporting tx mode
func (b *ConnectionBase) SupportTx() bool {
	return false
}
...
// SetKeyNameTag setter for fieldNameTag
func (b *ConnectionBase) SetKeyNameTag(name string) {
	b.keyNameTag = name
}

// KeyNameTag getter for fieldNameTag
func (b *ConnectionBase) KeyNameTag() string {
	if b.keyNameTag == "" {
		b.keyNameTag = "key"
	}
	return b.keyNameTag
...
func (b *ConnectionBase) Connect() error {
	return errors.New("Connect method is not yet implemented. It should be called from a driver connection object")
}
...
// ValidateTable in this base struct this method is not yet implemented, will be deprecated, please use EnsureDB on orm instead or combination of EnsureTable and EnsureIndex
func (b *ConnectionBase) ValidateTable(obj interface{}, autoUpdate bool) error {
	return fmt.Errorf("ValidateSchema is not yet implemented")
}

// DropTable in this base struct this method is not yet implemented
func (b *ConnectionBase) DropTable(name string) error {
	return fmt.Errorf("DropTable is not yet implemented")
}
...
func (b *ConnectionBase) Execute(c ICommand, m codekit.M) (interface{}, error) {
	q, err := b.Prepare(c)
	if err != nil {
		return nil, fmt.Errorf("unable to prepare query. %s", err.Error())
	}
...
func (b *ConnectionBase) Cursor(c ICommand, m codekit.M) ICursor {
	q, err := b.Prepare(c)
	if err != nil {
		//return nil, fmt.Errorf("usnable to prepare query. %s", err.Error())
		cursor := new(CursorBase)
		cursor.SetError(fmt.Errorf("unable to prepare query. %s", err.Error()))
		return cursor
	}
	cursor := q.Cursor(m)
...
// NewConnection
func NewConnection(schema, host, dbname, user, password string, config codekit.M) (IConnection, error) {
	driver := schema
	...
	return nil, fmt.Errorf("driver %s is unknown", driver)
}

// EnsureTable ensure table is exist with respective structure
func (b *ConnectionBase) EnsureTable(name string, keys []string, obj interface{}) error {
	return errors.New("EnsureTable method is not yet implemented. It should be called from a driver connection object")
}

// EnsureIndex to make sure index has been applied to the object
func (b *ConnectionBase) EnsureIndex(tableName, fieldName string, isUnique bool, fields ...string) error {
	return errors.New("EnsureIndex method is not yet implemented. It should be called from a driver connection object")
}

// HasTable check if table exist
func (b *ConnectionBase) HasTable(name string) bool {
	cmd := From(name).Select().Take(1)
	cs := b.This().Cursor(cmd, nil)
	defer cs.Close()
	return cs.Error() == nil
}
...
// Example URI: json://localhost/Users/sugab/sandbox/go/src/git.kanosolution.net/kano/dbflex/data?extension=json
func NewConnectionFromURI(uri string, config codekit.M) (IConnection, error) {
	si := new(ServerInfo)
	si.ConnectionString = uri
...
// SQL  set tableName
func SQL(command string) ICommand {
	return new(CommandBase).SQL(command)
}

var timeFormat string

func (conn *ConnectionBase) TimeFormat() string {
	if timeFormat == "" {
...
func (conn *ConnectionBase) SetTimeFormat(f string) {
	timeFormat = f
}
