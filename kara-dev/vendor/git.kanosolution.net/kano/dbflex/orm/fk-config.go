...
func checkFK(conn dbflex.IConnection, dm DataModel) error {
	fks := dm.FK()
	for _, fk := range fks {
		if e := ensureFK(conn, dm, fk.FieldID, fk.RefTableName, fk.RefField, fk.AutoCreate,
			fk.NumSeqID, fk.Map, fk.RefDefaultFields...); e != nil {
			return e
		}
	}
	return nil
...
func updateReverseFK(conn dbflex.IConnection, dm DataModel) error {
	...
	for _, fk := range fks {
		...
		if len(fields) > 0 {
			...
			if _, e := conn.Execute(cmd, codekit.M{}.Set("data", mUpdate)); e != nil {
