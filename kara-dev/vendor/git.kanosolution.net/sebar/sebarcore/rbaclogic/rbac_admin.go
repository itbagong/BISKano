...
func (obj *adminEngine) AddUserToRole(payload *RoleMember) (string, error) {
	...
	rm := RoleMember{}
	payload.Dimension.Hash = payload.Dimension.Items.String()
	if e := h.GetByFilter(&rm, dbflex.Eqs("UserID", payload.UserID, "RoleID", payload.RoleID, "Dimension.Hash", payload.Dimension.Hash)); e == nil {
		return "", nil
	}
	e := h.Save(payload)
