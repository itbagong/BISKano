<template>
    ...
    <s-card hide-footer hide-title class="card">
       <s-form v-if="data.config"  ref="frmCtl"
        ...
        <template #footer_2>
            <div class="mt-2 text-right text-primary cursor-pointer hover:text-secondary" @click="redirectApp()">I remember my credentials, bring me back to Login Page</div>
        </template>
        </s-form>
    </s-card>
    </div>
...
function redirectApp(cb){
    if(data.appID !==  '' &&  data.appID !== undefined){
        axios.post('/admin/app/get', [data.appID]).
            then(r => {   
                cb()
                location.href = r.data.Address + '/iam/login';
            }, 
            e =>  {  
                cb()
                router.push('/login')
