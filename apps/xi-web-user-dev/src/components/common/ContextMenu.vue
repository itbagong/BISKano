<template>
    <div v-if="submenu.length > 0">
        <Menu as="div" class="relative inline-block text-left">
            <MenuButton class="flex gap-2 items-center w-full" v-if="viewType == 'full'">
    ...
    <div v-else-if="viewType=='full'" class="flex gap-2 items-center w-full" @click="goto(url)">
        <mdicon :name="icon" size="16" class="w-[20px]" v-if="icon != ''"></mdicon>
        <div v-else class="w-[20px]">&nbsp;</div>
        <div class="grow text-left">{{ label }}</div>
    </div>
...
const props = defineProps({
    ...
    left: { type: String, default: "left-[180px]" },
