...
func ConfigToApp(configPath, portPrefix string) (*AppConfig, *kaos.HubManager, kaos.EventHub, error) {
	...
	e := appkit.ReadConfig(configPath, appConfig)
	if e != nil {
	...
	for k, v := range appConfig.Connections {
		...
		hconn := datahub.NewHub(datahub.GeneralDbConnBuilderWithTx(v.Txt, false), true, v.PoolSize)
	...
	hm.SetHubBuilder(func(key, group string) (*datahub.Hub, error) {
		...
		hconn := datahub.NewHub(datahub.GeneralDbConnBuilderWithTx(vTenantConnStr, false), true, vTenantPoolSize)
		hconn.SetAutoCloseDuration(2 * time.Second)
		return hconn, nil
	})
