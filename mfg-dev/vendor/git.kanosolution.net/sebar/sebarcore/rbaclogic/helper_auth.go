...
func (obj *authEngine) authenticate(ctx *kaos.Context, id, checkName, password string, secondLifeTime int) (*AuthResponse, error) {
	h := GetRbacDb(ctx)
	if h == nil {
	...
	// extract security matrix
	um, err := GetUserMatrix(h, user.ID, "")
	if err == nil {
		jwtData.Set("RBAC", um)
		sessionData.Set("RBAC", um)
	}

	expirySecond := secondLifeTime
	if expirySecond == 0 {
...
func JwtToSession(jwtCode, secret string) (string, codekit.M, error) {
	...
	if !tkn.Valid {
		return bc.Id, bc.Data, errors.New("jwt token is invalid")
	}
	if bc.StandardClaims.ExpiresAt < time.Now().UnixMilli() {
		return bc.Id, bc.Data, errors.New("jwt token is expired")
	}
	return bc.Id, bc.Data, nil
}

func GetRbacDb(ctx *kaos.Context) *datahub.Hub {
	db, _ := ctx.GetHub("iam", "")
	return db
}

func GetUserID(ctx *kaos.Context) string {
	return ctx.Data().Get("jwt_reference_id", "").(string)
}

func GetCompanyID(ctx *kaos.Context) string {
	return ctx.Data().Get("jwt_data", codekit.M{}).(codekit.M).GetString("CompanyID")
}

func GetTenantID(ctx *kaos.Context) string {
	return ctx.Data().Get("jwt_tenant_id", "").(string)
}
