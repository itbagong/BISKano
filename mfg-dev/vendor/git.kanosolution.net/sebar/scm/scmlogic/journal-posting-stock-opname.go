...
func (p *StockOpnameJournalPosting) ExtractLines() error {
	...
	for index, line := range p.header.Lines {
		...
		lineTrx.InventQty, err = ConvertUnit(p.db, (line.QtyActual - line.QtyInSystem), line.UnitID, lineTrx.Item.DefaultUnitID)
		if err != nil {
...
func (p *StockOpnameJournalPosting) Approve(op string, txt string) (string, error) {
	...
	switch pa.Status {
	case "REJECTED":
		p.header.Status = scmmodel.JournalStatus(pa.Status)
		p.db.Save(p.header)
...
func (p *StockOpnameJournalPosting) Post() error {
	...
	p.header.Status = scmmodel.JournalPosted
