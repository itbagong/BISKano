...
func (b *CommandBase) Command(command string, parm interface{}) ICommand {
	b.items[QueryCommand] = QueryItem{command, parm}
	return b
}
...
func PushVarToCommand(c ICommand, vars codekit.M) {
	items := c.Items()
	qis := QueryItems{}
	for k, v := range items {
		switch k {
...
func PushVarToFilter(f *Filter, vars codekit.M) {
