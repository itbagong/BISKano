...
func (o *StockOpnameEngine) Approve(ctx *kaos.Context, payload *StockOpnameApproveRequest) (interface{}, error) {
	...
	if len(gapLines) > 0 {
		inv := scmmodel.InventoryAdjustment{
			...
			Dimension:      so.FinancialDimension,
			InventDim:      so.InventoryDimension,
		}
...
func (o *StockOpnameEngine) GetLines(ctx *kaos.Context, payload *scmmodel.InventDimension) ([]scmmodel.StockOpnameJournalLine, error) {
	h := sebar.GetTenantDBFromContext(ctx)
	if h == nil {
		return nil, fmt.Errorf("missing: connection")
	}
	...
	if len(ibs) == 0 {
		return []scmmodel.StockOpnameJournalLine{}, nil
	}
	...
	res := []scmmodel.StockOpnameJournalLine{}
	for _, ib := range ibs {
		uom := ""
		uomName := ""
		if it, ok := itemM[ib.ItemID]; ok {
			uom = it.DefaultUnitID
			uomData, _ := uomORM.Get(uom)
			uomName = uomData.Name
		}

		desc := tenantcorelogic.ItemVariantName(h, ib.ItemID, ib.SKU)
		// if spec, ok := specM[fmt.Sprintf("%s|%s", ib.ItemID, ib.SKU)]; ok {
		// 	desc = formatItemDescription(h, spec)
		// }

		aisle, _ := aisleORM.Get(ib.InventDim.AisleID)
		section, _ := sectionORM.Get(ib.InventDim.SectionID)
		box, _ := boxORM.Get(ib.InventDim.BoxID)

		res = append(res, scmmodel.StockOpnameJournalLine{
