...
func CalcUnitCostNoDim(db *datahub.Hub, itemID string) float64 {
	wheres := []*dbflex.Filter{dbflex.Eqs("Item._id", itemID, "Status", scmmodel.ItemConfirmed), dbflex.Gt("Qty", 0)}

	trxs, _ := datahub.FindByFilter(db, new(scmmodel.InventTrx), dbflex.And(wheres...))
	unitCost := lo.SumBy(trxs, func(trx *scmmodel.InventTrx) float64 {
		return (lo.Ternary(trx.AmountFinancial == 0, trx.AmountPhysical, trx.AmountFinancial) + trx.AmountAdjustment) / trx.Qty
	})
