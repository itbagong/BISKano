...
func (p *InventoryTransferPosting) Calculate(opt ficologic.PostingHubExecOpt, header *scmmodel.InventJournal, lines []scmmodel.InventTrxLine) (*tenantcoremodel.PreviewReport, map[string][]orm.DataModel, float64, error) {
	...
	for index, line := range lines {
		...
		inventTrx.Status = scmmodel.ItemReserved // biar ke detect di Item Min Max Validation
	...
	for index, line := range lines {
		...
		inventTrx.AmountPhysical = line.UnitCost * inventTrx.Qty
		inventTrx.AmountFinancial = 0.0
		inventTrx.Status = scmmodel.ItemPlanned // biar ke detect di Item Min Max Validation
...
func (p *InventoryTransferPosting) Validate(lines []*scmmodel.InventTrx) error {
	...
	for _, line := range p.inventTrxLines {
		...
		if e := ValidateBalanceSpecific(p.opt.Db, line.Qty, line.UnitID, line.ItemID, line.SKU, p.header.CompanyID, line.InventDim); e != nil {
			return e
		}
