...
func ValidateBalanceSpecific(h *datahub.Hub, qty float64, unitID, itemID, sku, companyID string, inventDim scmmodel.InventDimension) error {
	inventDim.Calc()
	...
	balFilter.WarehouseIDs = append(balFilter.WarehouseIDs, inventDim.WarehouseID)
	balFilter.SectionIDs = append(balFilter.SectionIDs, inventDim.SectionID)
	balFilter.SKUs = append(balFilter.SKUs, sku)
