	...
	*/
	...
	go func(p *DbPooling) {
		...
		newPoolItem := func() bool {
			...
			//fmt.Println(len(p.items), "|", codekit.JsonString(p.items))
			p.Unlock()
		...
		for done := false; !done; {
			...
			for _, pi := range p.GetItems() {
				if pi != nil && pi.IsFree() && !pi.isClosed() {
					//fmt.Printfn("Connection is available, will be used. Size: %d Count: %d", p.Size(), p.Count())
					pi.retrieveDbPoolingInfo(p)
					pi.Use()
					cpi <- pi
					done = true
	...
	select {
	case pi := <-cpi:
		//fmt.Printfn("Connection is used. Size: %d Count: %d", p.Size(), p.Count())
		return pi, nil
	case err := <-cerr:
		return nil, fmt.Errorf("unable to get pool item. %s", err.Error())
	}
}
...
func (p *DbPooling) newItem() (*PoolItem, error) {
	...
	pi.retrieveDbPoolingInfo(p)
	//t0 := time.Now()
	//hashClose := codekit.ToInt(codekit.Date2String(time.Now(), "HHmmss"), codekit.RoundingAuto)*1000 + codekit.RandInt(1000)
