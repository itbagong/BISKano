...
func (dims Dimension) Hash() string {
	source := "no-dimension"
	if len(dims) > 0 {
		mapDims := lo.SliceToMap(dims, func(d DimensionItem) (string, string) {
			return d.Key, fmt.Sprintf("%s=%s", d.Key, d.Value)
		})
		keys := dims.Keys()
		sort.Strings(keys)
		source = strings.Join(lo.Map(keys, func(k string, index int) string {
			return mapDims[k]
		}), "&&")
	}
...
// Sub generate other dimension as subdimension from respective keys
func (dim Dimension) Sub(keys ...string) Dimension {
	subDim := Dimension{}
	for _, k := range keys {
		subDim = subDim.Set(k, dim.Get(k))
	}
	return subDim
}
