...
type UsageCheckResponse struct {
	IsUsed bool
	UsedIn []UsageCheckResUsedIn
}

type UsageCheckResUsedIn struct {
	Type      string
	JournalID string
}

func MWPreItemDelete() kaos.MWFunc {
	return func(ctx *kaos.Context, payload interface{}) (bool, error) {
		m := new(tenantcoremodel.Item)
		if e := serde.Serde(payload, &m); e != nil {
			return false, fmt.Errorf("failed to parse payload: %s", e.Error())
		}

		h := sebar.GetTenantDBFromContext(ctx)
		if h == nil {
			return false, fmt.Errorf("missing: db connection")
		}
		...
		if e := ev.Publish("/v1/scm/item/usage-check", &struct{ ItemID string }{ItemID: m.ID}, resp, nil); e != nil {
			return false, fmt.Errorf("failed to check item usage: %s", e.Error())
		}

		if resp.IsUsed {
			return false, fmt.Errorf("item is used in %d journal(s)", len(resp.UsedIn))
		}
...
func MWPreLogSave() kaos.MWFunc {
	return func(ctx *kaos.Context, payload interface{}) (bool, error) {
		...
		action := tenantcoremodel.LogActionUpdate
		if m.GetString("_id") == "" {
			action = tenantcoremodel.LogActionCreate
		}
		ctx.Data().Set("LogAction", action)

		return true, nil
	}
}

func MWPostLogSave(menu tenantcoremodel.LogMenu) kaos.MWFunc {
	return func(ctx *kaos.Context, payload interface{}) (bool, error) {
		res, ok := ctx.Data().Data()["FnResult"]
		if !ok {
			return true, nil
		}

		item := new(tenantcoremodel.Item)
		if e := serde.Serde(res, item); e != nil {
			fmt.Println("MWPostLogAdd() ERROR SERDE:", e)
			return true, nil
		}

		action := ctx.Data().Get("LogAction", "").(tenantcoremodel.LogAction)

		logParam := tenantcoremodel.LogParam{
			Hub:           sebar.GetTenantDBFromContext(ctx),
			Menu:          string(menu),
		...
		new(tenantcoremodel.Log).Add(logParam)

		return true, nil
	}
}

func MWPreLogDelete() kaos.MWFunc {
	return func(ctx *kaos.Context, payload interface{}) (bool, error) {
		m := codekit.M{}
		if e := serde.Serde(payload, &m); e != nil {
			return true, nil
		...
		ctx.Data().Set("LogAction", tenantcoremodel.LogActionDelete)
		ctx.Data().Set("LogTransactionID", m.GetString("_id"))
		ctx.Data().Set("LogName", m.GetString("Name"))
		if m.GetString("Name") == "" {
			ctx.Data().Set("LogName", m.GetString("Text"))
		}
...
func MWPostLogDelete(menu tenantcoremodel.LogMenu) kaos.MWFunc {
	return func(ctx *kaos.Context, payload interface{}) (bool, error) {
		action := ctx.Data().Get("LogAction", "").(tenantcoremodel.LogAction)
		transID := ctx.Data().Get("LogTransactionID", "").(string)
		name := ctx.Data().Get("LogName", "").(string)
