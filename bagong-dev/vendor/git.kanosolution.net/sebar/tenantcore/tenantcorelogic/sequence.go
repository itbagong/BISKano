...
func mapSequence(ctx *kaos.Context, payload NumSeqClaimPayloadV2) (result string, err error) {
	...
	// for converting month to rome format
	...
	for _, c := range words {
		...
		if ok {
			if fSeq != "counter" && fSeq != "dt" && fSeq != "dim" && fSeq != "roman_month" {
				totalCustom++
			}
		}
	}
	...
	for _, c := range words {
		...
		if ok || fSeq == "roman_month" || fSeq == "tdc_title" {
			key := "${" + c + "}"
			switch fSeq {
			case "counter":
				resp := new(NumSeqClaimRespond)
			...
			case "roman_month":
				mapFormat[key] = romanMonths[time.Now().Month()]
			case "tdc_title":
				trainingTitleID := []string{payload.TrainingTitle}
				res := tenantcoremodel.SiteTenantBagong{}
				e := ev.Publish("/v1/hcm/tdctitle/get", &trainingTitleID, &res, nil)
				if e != nil {
					return result, e
				}
				mapFormat[key] = res.Alias
...
func MWPreAssignCustomSequenceNo(sequenceKind string) kaos.MWFunc {
	return func(ctx *kaos.Context, payload interface{}) (bool, error) {
		...
		mapSeq, err := mapSequence(ctx, NumSeqClaimPayloadV2{
			...
			TrainingTitle:    m.GetString("TrainingTitle"),
		})
		if err != nil {
