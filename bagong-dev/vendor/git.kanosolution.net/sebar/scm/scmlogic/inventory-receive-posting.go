		...
		if line.SourceTrxType == string(scmmodel.InventReceive) {
			...
			gis, _ := datahub.Find(p.opt.Db, new(scmmodel.InventTrx),
				dbflex.NewQueryParam().
					SetWhere(
						dbflex.Eqs("CompanyID", p.header.CompanyID, "SourceType", line.SourceType, "SourceJournalID", line.SourceJournalID, "SourceLineNo", line.LineNo, "SourceTrxType", scmmodel.InventIssuance, "Status", scmmodel.ItemConfirmed),
					).
					SetSort("-_id"),
			)
			if len(gis) == 0 {
