...
type GetUnitCostResponse struct {
	ItemID string
	Cost   float64
}

func (o *ItemEngine) GetUnitCost(ctx *kaos.Context, ps []GetUnitCostParam) ([]GetUnitCostResponse, error) {
	h := sebar.GetTenantDBFromContext(ctx)
	if h == nil {
		return nil, errors.New("missing: connection")
	}
	...
	invJourORM := sebar.NewMapRecordWithORM(h, new(scmmodel.InventJournal))

	res := []GetUnitCostResponse{}
	for _, p := range ps {
		item, _ := itemORM.Get(p.ItemID)

		if p.SourceJournalID != "" {
			j, _ := invJourORM.Get(p.SourceJournalID)
			if j.ID != "" && j.InventDim.WarehouseID != "" {
				p.InventDim = j.InventDim
			}
		}

		cost := GetCostPerUnit(h, *item, p.InventDim, p.TrxDate)

		res = append(res, GetUnitCostResponse{
			ItemID: p.ItemID,
			Cost:   cost,
		})
	}
...
func (o *ItemEngine) UsageCheck(ctx *kaos.Context, p *UsageCheckParam) (*UsageCheckResponse, error) {
	h := sebar.GetTenantDBFromContext(ctx)
	if h == nil {
		return nil, errors.New("missing: connection")
	}
	...
	h.GetsByFilter(new(scmmodel.InventJournal), dbflex.Eq("Lines.ItemID", p.ItemID), &inventJs)
	...
	h.GetsByFilter(new(scmmodel.PurchaseOrderJournal), dbflex.Eq("Lines.InventJournalLine.ItemID", p.ItemID), &poJs)
	...
	h.GetsByFilter(new(scmmodel.PurchaseRequestJournal), dbflex.Eq("Lines.InventJournalLine.ItemID", p.ItemID), &prJs)
	...
	h.GetsByFilter(new(scmmodel.ItemRequestDetail), dbflex.Eq("ItemID", p.ItemID), &irDetails)
