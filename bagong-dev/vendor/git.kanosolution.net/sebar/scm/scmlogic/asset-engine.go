...
func (o *AssetEngine) Upload(ctx *kaos.Context, req *AssetUploadRequest) (interface{}, error) {
	...
	case string(scmmodel.ModuleInventory):
		m = new(scmmodel.InventJournal)
	case string(scmmodel.InventReceive):
		m = new(scmmodel.InventReceiveIssueJournal)
	case string(scmmodel.InventIssuance):
		m = new(scmmodel.InventReceiveIssueJournal)
	case string(scmmodel.JournalTransfer):
		m = new(scmmodel.InventJournal)
	case string(scmmodel.ItemRequestType):
		m = new(scmmodel.ItemRequest)
	case string(scmmodel.AssetAcquisitionTrxType):
		m = new(scmmodel.AssetAcquisitionJournal)
	default:
