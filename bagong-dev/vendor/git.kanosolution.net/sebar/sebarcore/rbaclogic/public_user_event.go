...
type UpdateDimensionRequest struct {
	EmployeeID        string
	EmployeeDimension Dimension
}

func (obj *publicUserEvent) UpdateDimension(ctx *kaos.Context, payload *UpdateDimensionRequest) (string, error) {
	res := "fail"

	if payload == nil {
		return res, fmt.Errorf("no payload provided")
	} else {
		if payload.EmployeeID == "" {
			return res, fmt.Errorf("no EmployeeID provided")
		}
	}
	...
	user := new(User)
	if e := h.GetByID(user, payload.EmployeeID); e != nil {
		return res, fmt.Errorf("user not found: %s", payload.EmployeeID)
	}

	user.Dimension = payload.EmployeeDimension

	if e := h.Save(user); e != nil {
		return res, e
	}
...
func (obj *publicUserEvent) UpdateUser(ctx *kaos.Context, payload *UpdateUserRequest) (string, error) {
	res := "fail"
...
func sendUserActivation(ctx *kaos.Context, user *rbacmodel.User) error {
	...
	activationLink := fmt.Sprintf("%s%s?Email=%s", addrWebUser, addrActivationUserLink, user.Email)
