...
func NewCashTrxSplitter(db *datahub.Hub) *TrxSplitter[*ficomodel.CashTransaction, CashSplitOpt] {
	c := new(TrxSplitter[*ficomodel.CashTransaction, CashSplitOpt])
	c.db = db
	c.provider = new(CashSplitProvider)
	return c
}

type CashSplitProvider struct {
}

func (c *CashSplitProvider) GetTransactions(db *datahub.Hub, opt CashSplitOpt) ([]*ficomodel.CashTransaction, error) {
	...
	txs, err := datahub.FindByFilter(db, new(ficomodel.CashTransaction), dbflex.And(wheres...))
	if err != nil {
		return nil, err
	}
	return txs, nil
...
func (c *CashSplitProvider) Split(
	db *datahub.Hub,
	sources []*ficomodel.CashTransaction,
	amt float64, newStatus string,
	opt CashSplitOpt) ([]*ficomodel.CashTransaction, []*ficomodel.CashTransaction, error) {

	sourceTrxs := []*ficomodel.CashTransaction{}
	newTrxs := []*ficomodel.CashTransaction{}
	...
	for _, source := range sources {
		...
		newTrx := new(ficomodel.CashTransaction)
		*newTrx = *source
		newTrx.ID = ""
