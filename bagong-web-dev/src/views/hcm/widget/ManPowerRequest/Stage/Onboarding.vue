...
<div v-else>
    <status-text :txt="data.record.Status"/>
     <s-form ref="frmCtl" v-model="data.record" :config="data.frmCfg" keep-label
        ...
        <template #input_Checklist="{ item }"> 
            <Checklist  v-model="item.Checklist" />
      </template>
    </s-form>
</div>
</template>
...
function fetchRecord(){
    data.loading = true
    axios.post("/hcm/onboarding/get", [props.id]).then((r)=>{
        data.record = r.data
    }).catch(e=>{
        util.showError(e)
    }).finally(()=>{
...
function submitForm(record,cbOk, cbError){
    axios.post("/hcm/onboarding/update", record).then((r)=>{
        cbOk()
    }).catch(e=>{
        cbError()
        util.showError(e)
...
onMounted(()=>{
    loadFormConfig(axios, "/hcm/onboarding/formconfig").
        then(r => {
            data.frmCfg = r 
            fetchRecord()
        }, e => util.showError(e))
