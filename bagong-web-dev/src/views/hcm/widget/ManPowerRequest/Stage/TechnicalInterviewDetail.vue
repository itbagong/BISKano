    ...
    <table class="w-full"> 
        ...
        <tbody v-else>
            <tr v-for="(record, idx) in data.records" :key="idx" class="border [&>*]:p-2 [&>*]:border-r">
                <td valign="center" class="text-center  w-[40px] max-w-[70px]">{{record.Number}}</td>
                        ...
                        <div v-for="(child, idx2) in record.Detail" :key="idx2" class="px-2 py-1" > 
                            ...
                            <input
                                ...
                                (e) => {
                                    ...
                                    onChangeScoreMlc(idx, idx2, value)
                                }
                                "
                            />
                        </div>
...
function calcAverageMlc(idx, idxChild,v1){
    if (v1) {
        data.records[idx].Detail[idxChild].AverageMlc = parseFloat((v1 / 4  * data.records[idx].Detail[idxChild].Weight).toFixed(1))
    }
}
...
function onChangeScoreMlc(idx,idxChild, v1){
    let _v1 = v1
    if(_v1 > 4){
        util.nextTickN(2, () => {
            data.records[idx].Childs[idxChild].ScoreMlc = 4
        })
        _v1 = 4
    }
