<template>
  <data-list
    ...
    :grid-fields="['OffsetAccount','Amount','Text','Critical']"
  ...
  >
    <template #grid_OffsetAccount="{item}">
      <AccountSelector
        ...
        :items-type="journalType === 'Cash In' ? ['LEDGERACCOUNT','CUSTOMER'] : ['LEDGERACCOUNT','VENDOR','EXPENSE']"
        :read-only="readOnly"
      ></AccountSelector>
    </template>
...
function onAlterGridConfig(config) {
    const arr = ['OffsetAccount','Amount','Text','Critical'];
    if(props.status === 'SUBMITTED'){
        arr.push("ApproveAmount")
    }
