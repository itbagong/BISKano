    ...
    >
      ...
      <template #item_Qty="{ item }">
        <s-input
          ref="refQTY"
          v-model="item.Qty"
          :disabled="!['Open', 'PartialReceived'].includes(props.item.Status)"
          kind="number"
          class="w-full"
        ></s-input>
      </template>
...
function newRecordBatch() {
  ...
  record.GoodReceiptID = props.itemID;
...
function onSelectDataBatch(record, index) {
  ...
  if (record.BatchID != "" && record.Qty > 0) {
    ...
    for (let i = 0; i < record.Qty * record.Ratio; i++) {
      listBatch.push({
        GoodReceiptID: record.GoodReceiptID,
        ItemID: record.ItemID,
...
onMounted(() => {
  ...
  loadGridConfig(axios, "/scm/good-receipt/batch/gridconfig").then(
    (r) => {
      data.gridCfgBatch = r;
    },
    (e) => util.showError(e)
  );
  loadGridConfig(axios, "/scm/good-receipt/serial/gridconfig").then(
    (r) => {
