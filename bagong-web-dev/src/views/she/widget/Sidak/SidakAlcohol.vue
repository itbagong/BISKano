<template>
  <data-list
    ...
    hide-title
    no-gap
    form-config="/she/sidak/alcohol/formconfig"
    form-keep-label
  ...
  >
    <template #form_input_Sign="{ item, config }">
      <div class="hidden">{{ (config.label = "Alcohol Sign") }}</div>
      <uploader
        ref="gridAttachmentSign"
        :journalId="jurnalId"
        :config="config"
        journalType="SHE_ALCOHOL_SIGN"
        :key="3"
        single-save
      />
    </template>
    <template #form_input_Evidance="{ item, config }">
      <div class="hidden">{{ (config.label = "Alcohol Evidance") }}</div>
      <uploader
        ref="gridAttachmentEvidance"
        :journalId="jurnalId"
        :config="config"
        journalType="SHE_ALCOHOL_EVIDANCE"
        :key="4"
        single-save
      />
    </template>
