...
apiAxios.interceptors.request.use(async function (cfg) {
    const auth = authStore();
    auth.validate();
    if (auth.appToken != '') {
        cfg.headers.common['Authorization'] = 'Bearer ' + auth.appToken
    }
    return cfg
})
...
apiAxios.interceptors.response.use(r => {
    if (r.data && r.data.error) {
        ...
        if (errorMessage == "EOF") {
            return Promise.reject("Sorry, but the data you looking for is not exist")
