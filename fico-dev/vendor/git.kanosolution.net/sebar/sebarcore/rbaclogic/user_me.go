...
func (obj *UserMeLogic) Activate(ctx *kaos.Context, payload *ActivateRequest) (codekit.M, error) {
	res := codekit.M{}
	...
	user := rbacmodel.User{}
	if err := h.GetByFilter(&user, dbflex.Eq("Email", payload.Email)); err != nil {
	...
	where := dbflex.Eqs("UserID", user.ID, "Kind", "rbac-new-user", "Status", "Reserved")
...
func (obj *UserMeLogic) ResendActivationEmail(ctx *kaos.Context, payload *ActivateRequest) (codekit.M, error) {
	res := codekit.M{}
...
func (obj *UserMeLogic) ChangePasswordWithoutUserID(ctx *kaos.Context, payload *ChangeUserPasswordRequest) (string, error) {
	h, _ := ctx.DefaultHub()
	if h == nil {
		return "", errors.New("missing: dbconn")
	}
	...
	vAdminEngine := NewAdminEngine(h, ev)
	vUserID, err := vAdminEngine.ChangeUserPassword(h, payload)
	if err != nil {
		return "", errors.New("error when change password: " + err.Error())
	}
