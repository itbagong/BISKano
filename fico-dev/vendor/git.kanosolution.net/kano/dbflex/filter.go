...
const (
	...
	// OpEq is Equal
	OpEq = "$eq"
	// OpEqs is Equal to multi fields
	OpEqs = "$eqs"
	// OpNe is Not Equal
	OpNe = "$ne"
	...
	// OpNin is Not in
	OpNin = "$nin"
	// OpAll contains this element in array
	OpAll = "$all"
	// OpElemMatch element contain this informaiton
...
func And(items ...*Filter) *Filter {
	...
	return NewFilter("", OpAnd, nil, validItems)
}
...
// Eqs create new filter with Eqs operation
func Eqs(vs ...interface{}) *Filter {
	fs := []*Filter{}
	var f *Filter
	for _, v := range vs {
		if f == nil {
...
func In[T any](field string, invalues ...T) *Filter {
	f := new(Filter)
	f.Field = field
	f.Op = OpIn
	f.Value = invalues
...
// All create new filter with all operator
...
// ElemMatch create new filter with elemMatch operator
func ElemMatch(field string, matches ...*Filter) *Filter {
	f := new(Filter)
	f.Field = field
	f.Op = OpElemMatch
	f.Items = matches
