...
func (obj *FixedAssetNumberHandler) SaveFixedAssetNumber(h *datahub.Hub, fan *ficomodel.FixedAssetNumber, ev kaos.EventHub, isInsert bool) error {
	...
	mapFANCodeTotalRollback := map[string]int{}
	...
	if len(fan.Details) > 0 {
		...
		for _, val := range fan.Details {
			...
			for i := 0; i < val.NumberAsset; i++ {
				...
				// for rollback fancode
				if vv, ok := mapFANCodeTotalRollback[val.FixedAssetGrup]; ok {
					mapFANCodeTotalRollback[val.FixedAssetGrup] = vv + 1
				} else {
					mapFANCodeTotalRollback[val.FixedAssetGrup] = 1
				}
			}
		...
		if len(resultList) > 0 {
			...
			if isInsert {
				...
				res := ""
				err = ev.Publish("/v1/bagong/asset/add-asset", &payload, &res, nil)
				// fmt.Printf("asset id: %s | asset/insert e: %s\n", IDs, err)
				if err != nil {
					e := RollbackFixedAsset(h, fan, mapFANCodeTotalRollback, IDs)
					if e != nil {
						return errors.New("error rollback fixed asset number: " + e.Error())
					}
					return errors.New("error when insert bg asset. " + err.Error())
				}
			}
		}
	}
...
func RollbackFixedAsset(h *datahub.Hub, fan *ficomodel.FixedAssetNumber, mapFANCodeTotalRollback map[string]int, IDs []string) error {
	// delete tenant asset and fixed asset number list
	if len(IDs) > 0 {
		err := h.DeleteQuery(new(ficomodel.FixedAssetNumberList), dbflex.In("_id", IDs...))
		if err != nil {
			return errors.New("error when delete fixed asset number list" + err.Error())
		}

		err = h.DeleteQuery(new(tenantcoremodel.Asset), dbflex.In("_id", IDs...))
		if err != nil {
			return errors.New("error when delete tenant asset" + err.Error())
		}
	}

	// update seq
	if len(mapFANCodeTotalRollback) > 0 {
		fixedAssetNumberCodes := []ficomodel.FixedAssetNumberCode{}
		err := h.Gets(new(ficomodel.FixedAssetNumberCode), nil, &fixedAssetNumberCodes)
		if err != nil {
			return errors.New("Failed populate data fixed asset number code: " + err.Error())
		}

		mapFixedAssetNumberCode := lo.Associate(fixedAssetNumberCodes, func(fancode ficomodel.FixedAssetNumberCode) (string, ficomodel.FixedAssetNumberCode) {
			return fancode.ID, fancode
		})

		for key, val := range mapFANCodeTotalRollback {
			fixedAssetNumberCode := ficomodel.FixedAssetNumberCode{}
			// get value for mapFixedAssetNumberCode
	...
	err := h.Delete(fan)
	if err != nil {
		return errors.New("error delete fixed asset number")
	}
