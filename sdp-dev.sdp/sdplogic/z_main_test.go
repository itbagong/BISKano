...
var (
	...
	testDbPort = 27888
	natsPort   = 4222
	svc        = kaos.NewService().SetBasePoint("/v1/sdp")
...
func TestMain(m *testing.M) {
	...
	dbSvr, err = strikememongo.StartWithOptions(&strikememongo.Options{
		...
		MongodBin: "C:\\Program Files\\MongoDB\\Server\\7.0\\bin\\mongod.exe",
		Port:      testDbPort,
		// LogLevel: strikememongolog.LogLevelDebug,
		LogLevel:       strikememongolog.LogLevelInfo,
		StartupTimeout: 20 * time.Second,
		TempDirFolder:  "C:\\tempmongo",
	})
	if err != nil {
		log.Fatal(err)
	}
	...
	sdplogic.RegisterJournalAPI(svc, uimod, dbmod)
	svc.PrepareRoutes("")
	routes := svc.Routes()
	...
	fns := []injectDataFn{injectLedgerBankMasterData, injectLedgerBankConfigData, injectUnit}
	for _, fn := range fns {
