<template>
  <div
    :class="{
      notifClearLayout: layout.name == 'clear',
      notifTenantLayout: layout.name == 'tenant' || layout.name == 'admin',
    }"
  >
    <div
      v-for="msg in messages"
      class="w-full text-center items-center justify-center flex gap-1 p-2 opacity-70"
      :class="{
        btn_secondary: msg.kind == 'info',
        btn_warning: msg.kind == 'warning',
        btn_error: msg.kind == 'error',
      }"
    >
      <div class="grow">{{ msg.message }}</div>
      <div
        class="cursor-pointer hover:bg-primary hover:text-white"
        @click="notif.remove(msg.msgid)"
      >
