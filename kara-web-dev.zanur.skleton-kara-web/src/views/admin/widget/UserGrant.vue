<template>
    <div class=" w-full">
        <data-list ref="grid" hide-title no-gap
...
function openForm(r) {
    util.nextTickN(2,()=>{
        ...
        grid.value.setFormFieldAttr("TenantID","lookupUrl",`/admin/tenantuser/find?UserID=${props.user._id}`)
...
function alterGridConfig(cfg) {
    cfg.fields = cfg.fields.filter(el => el.field != 'UserID')
    //console.log(cfg)
}
